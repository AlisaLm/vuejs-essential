    ...
    function getErrorElement(el) {
        ...
        if (!errorElement) {
            ...
            const fragement = document.createRange().createContextualFragment(tpl)
    ...
    export default {
        bind(el, binding, vnode) {
            ...
            // 使用解构赋值声明 value = binding.value,arg = binding.arg,modifiers = dinding.modifiers
            const {value,arg,modifiers} = binding
            // 如果没传对应的时间名称参数，就默认使用change事件
            const eventType = ['change','blur','input'].indexOf(arg) !== -1 ? arg : 'change'
            // 默认处理器，当用户开始输入时，移除错误提示
            const defaultHandler = () => {
                showError(el)
            }
            // 验证处理器，当用户触发对应当事件时，验证用户输入的信息
            const handler = () => {
        ...
        inserted(el,binding,vnode) {
            ...
            // 指定当前一系列验证项的父级，我们这里指定为含data-validator-form的元素
            const form = el.closest('[data-validator-form]')
            // 指定一个按钮来检查所有验证项，我们这里指定为含type=submit的元素
            ...
            if (submitBtn) {
                ...
                const submitHandler = () => {
                    ...
                    if (!errors.length) {
                        // 没有错误信息时，在按钮上添加一个canSubmit属性，并指定为true
                        submitBtn.canSubmit = true
                    } else {
                        // 有错误信息时，在按钮上添加一个canSubmit属性，并指定为false
